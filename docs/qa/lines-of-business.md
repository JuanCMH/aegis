...
Salud, etc.). Cada ramo puede tener un **código corto** (auto-uppercase),
una **comisión por defecto** (%) que se precargará al crear pólizas del
ramo, y una **descripción** opcional. Admite archivar para ocultar sin
eliminar.
...
- `Agencia Demo` con ramos seed:
  - "Autos" (código AUTO, comisión 15%)
  - "Vida" (código VIDA, comisión 20%)
  - "Hogar" (código HOGAR, comisión 12%)
...
| 2 | Observar columnas                 | Ramo (icon+nombre+desc), Código (mono), Comisión (mono %), Estado (badge Activo emerald), menú (⋯) |
...
| 3 | Escribir "sal" en Código                                    | Input convierte a "SAL" en tiempo real (uppercase)        |
...
| 2 | Escribir "VIDA"                                             | Match por código                                           |
...
| 5.3  | Crear con código duplicado                                  | Toast "Ya existe un ramo con ese código"                  |
...
| 5.10 | Editar sin cambios                                          | Mutation idempotente; toast "Ramo actualizado"            |
| 5.11 | Doble click "Crear"                                         | Botón disabled mientras `isPending`                       |
| 5.12 | Usuario sin `linesOfBusiness_manage`                        | "Nuevo ramo" oculto; menú (⋯) oculto en filas             |
| 5.13 | Usuario sin `linesOfBusiness_view`                          | Query devuelve `[]`, empty state                          |
| 5.14 | Eliminar último ramo                                        | Empty state con CTA (si `_manage`)                        |
