...
import { FIELD_GRID_CLASSES } from "@/packages/template-builder/lib/grid";
import type {
  TemplateSection,
  TemplateValues,
...
interface DynamicStepperProps {
  ...
  /**
   * Optional URL to the template builder. When provided and a section has no
   * fields, an empty-state link points users to the builder. Each consumer
   * passes its own settings route (clients, policies, etc.).
   */
  templateSettingsHref?: string;
}

/**
 * Stepper genérico de plantilla dinámica. No asume ningún dominio.
 * El consumidor pasa `templateSettingsHref` con la ruta del builder
 * correspondiente (clients/settings, policies/settings, etc.).
 */
